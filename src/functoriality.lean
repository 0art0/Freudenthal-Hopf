...
def weak_end_hom.to_weak_hom (φ : V → V') (hφ : weak_end_hom G G' φ)
  (L : finset V') : (G.out (hφ.fin_map L)) ↝g (G'.out L) :=
    ⟨φ, λ _ _, hφ.reachable_out L⟩
...
begin
  ...
  fsplit,
  {
    ...
    let K := hφ.fin_map L,
    obtain ⟨C, Cdis⟩ := s K,
    let φ' := weak_end_hom.to_weak_hom _ _ φ hφ L,
    exact φ'.component_map _ _ C,

    sorry,
  },
  intros L L' h,
  apply subtype.ext_val,
  dsimp [ComplComp, category_theory.functor.map],
  sorry
end
